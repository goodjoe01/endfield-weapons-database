        ...
        <Link href={`/weapon-detail/${weapon.id}`} className="block">
          ...
          >
            ...
            <div className="px-3 pb-3 flex flex-col h-24">
              {/* Weapon Type Badge */}
              <div className="text-xs text-gray-100 mb-1 line-clamp-1 overflow-hidden text-ellipsis">{getDisplayWeaponType(weapon.weaponType ?? '')}</div>
              ...
              {/* Name */}
              <h3 className="font-semibold text-foreground line-clamp-2 text-sm flex-1 overflow-hidden text-ellipsis">{weapon.name}</h3>
            </div>
          </div>
        </Link>
